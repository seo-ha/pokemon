...
const Pokemon = () => {
  ...
  const search = (e) => {
    ...
    if (searchInput.length >= 0) {
      var searchDataList = searchPokemonData.filter((item) =>
        item.korean_name.includes(e.target.value)
      );
  ...
  return (
    <div className="content">
      <input
        className={searchPokemonData.length > 0 ? "on" : ""}
        type="search"
        name=""
        id="search"
        placeholder={searchPokemonData.length > 0 ? "search..." : "loading.."}
        value={searchInput}
        onChange={(e) => search(e)}
        disabled={searchPokemonData.length > 0 ? false : true}
      />
        ...
        <InfiniteScroll
          ...
          dataLength={
            searchInput.length <= 0
              ? pokemonData.length
              : searchPokemonLenght.length
          }
          next={searchInput.length <= 0 ? fetchMoerData : ""}
          hasMore={
            searchInput.length <= 0 ? currentpage * pokemonPerPage < 151 : ""
          }
          loader={
                      ...
                      >
                        ...
                        <div className="types">
                          {item.type.map(({ type }, idx) => {
                            return (
                              <span key={idx} className={type.name}>
                                {type.korean_name}
                              </span>
                            );
                          })}
